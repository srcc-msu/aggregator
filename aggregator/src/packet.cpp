...
double GetDiv(const UValue& v1, const UValue& v2, e_sens_type type
	, size_t msg_length)
{
	if(v1.b8[0] == v2.b8[0])
		return 1.0;
	...
	if(type == INTEGER || type == WIRECOUNTER || type == WIRESMART)
	{
		if(msg_length == 1) res = (v2.b1[0] != 0)
			? double(v1.b1[0]) / v2.b1[0] : INF;
		if(msg_length == 2) res = (v2.b2[0] != 0)
			? double(v1.b2[0]) / v2.b2[0] : INF;
		if(msg_length == 4) res = (v2.b4[0] != 0)
			? double(v1.b4[0]) / v2.b4[0] : INF;
		if(msg_length == 8) res = (v2.b8[0] != 0)
			? double(v1.b8[0]) / v2.b8[0] : INF;
	}
	else if(type == WIREFLOAT)
	{
		if(msg_length == 4) res = (fabs(v2.f4[0]) > EPS)
			? v1.f4[0] / v2.f4[0] : INF;
		if(msg_length == 8) res = (fabs(v2.f8[0]) > EPS)
			? v1.f8[0] / v2.f8[0] : INF;
	}
	else
		throw CException("unsupported packet type during parsing");
...
double GetDiff(const UValue& v1, const UValue& v2, e_sens_type type
	, size_t msg_length)
{
	if(v1.b8[0] == v2.b8[0])
		return 1.0;
...
UValue GetSum(const UValue& v1, const UValue& v2, e_sens_type type
	, size_t msg_length)
{
	UValue res;
...
UValue MultValue(const UValue& value, e_sens_type type, size_t msg_length
	, double mult)
{
	UValue res;
